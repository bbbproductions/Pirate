...
{
	public class WoodenBallista : ModProjectile
	{
		...
		public override void AI() {
			const int ShootFrequency = 90; // How long the sentry waits between shots.
			const int TargetingRange = 30 * 9; // The sentry's targeting range, 50 tiles.
			const float FireVelocity = 7f; // The velocity the sentry's shot projectile will travel.
			...
			if (targetNPC != null) {
				if (ShootTimer <= 0) {
					...
					if (Main.myPlayer == Projectile.owner) {
						...
						int type = ModContent.ProjectileType<WoodenBallistaBolt>();
		...
		public override void OnKill(int timeLeft) {
			...
			for (int i = 0; i < 50; i++) {
				Vector2 speed = Main.rand.NextVector2CircularEdge(1f, 1f);
				Dust d = Dust.NewDustPerfect(Projectile.Center + speed * 50, DustID.Smoke, speed * -5, Scale: 1.5f);
				d.noGravity = true;
			}
		}
	}
